...
- 术语
  - 平台：优惠券交易平台
  - 漏洞：给`平台`造成损失的情况。比如银行和公安会进行监管的地方
- 描述：为了分析、预估`平台`的风险,我们需要成立一个风险分析小组。从业务逻辑、`平台`交易等不同方面进行考虑，查找`平台`漏洞并提出初步的解决方案。
- 针对性：主要从优惠券交易`平台`项目的交易方面来进行分析。
  - 例子：
    - 例如：买家A购买了卖家B的优惠券，买家A已使用了优惠券，却在`平台`进行申请仲裁，仲裁投票结果为买家A胜利，结果`平台`会将钱退给买家A，给卖家B带来损失的情况。
    - 方案：需要我们思考
- 内容：
