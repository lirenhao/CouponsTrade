...
app.post(`/${ServerPath.EDIT_USER_COUPON}`, function (req, res) {
    ...
    if (state.token === token) {
        let couponInfo = state.publishCouponList.filter((r)=> {
            return r.id === id
        })[0]
        if (Object.keys(req.body).length === 2) {
            couponInfo = {...couponInfo, couponState: "1"}
        } else {
            couponInfo = {
                id,
                couponName,
        ...
        const resultCouponInfo = state.publishCouponList.filter((r)=> {
            return r.id === id
        })[0]
        res.json({code: ResponseCode.SUCCESS, couponInfo: resultCouponInfo})
    } else {
